...
class Cms extends TagLib
{
    ...
    protected $tags = [
        ...
        'channelgroup' => ['attr' => 'channel', 'close' => 1],
        'yantaolist' => ['attr' => 'id,empty,key,mod,orderby,orderway,row,date', 'close' => 1],

    ];
    public function tagChannelgroup($tag, $content){
        ...
        $var = Random::alnum(10);
        $parse = '<?php ';
        $parse .= '$__' . $var . '__ = \addons\cms\model\Tags::getTagGroup([' . implode(',', $params) . ']);';
        $parse .= ' ?>';
    ...
    public function tagYantaoList($tag, $content){
        $id = isset($tag['id']) ? $tag['id'] : 0;
        $empty = isset($tag['empty']) ? $tag['empty'] : '';
        $key = !empty($tag['key']) ? $tag['key'] : 'i';
        $mod = isset($tag['mod']) ? $tag['mod'] : '2';
        ...
        $var = Random::alnum(10);
        $parse = '<?php ';
        $parse .= '$__' . $var . '__ = \addons\cms\model\Archives::getYantaoList([' . implode(',', $params) . ']);';
        $parse .= ' ?>';
        $parse .= '{volist name="$__' . $var . '__" id="' . $id . '" empty="' . $empty . '" key="' . $key . '" mod="' . $mod . '"}';
        $parse .= $content;
        $parse .= '{/volist}';
