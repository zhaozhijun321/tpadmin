...
Link.prototype = {
    ...
    _createPanel: function _createPanel(text, link) {
        ...
        var btnSelId = getRandom('btn-select');
        ...
        var panel = new Panel(this, {
            ...
            }, // tab end
            {
                ...
                title: '选择附件',
                // 模板
                tpl: '<div>\n<div class="w-e-button-container">\n   <button id="' + btnSelId + '" class="">选择附件</button>\n                                </div>\n                        </div>',
                // 事件绑定
                events: [
                    // 选择附件
                    {
                        ...
                        fn: function fn() {
                            ...
                            parent.Fast.api.open("general/attachment/select?element_id=&multiple=false&mimetype=*", "选择", {
                                callback: function (data) {
                                    var urlArr = data.url.split(/\,/);
                                    urlArr.forEach(function (item, index) {
                                        var url = Fast.api.cdnurl(item);
                                        editor.cmd.do('insertHTML', "<a href='" + url + "'>" + url + "</a>");
                                    });
                                }
                            });
...
Image.prototype = {
    ...
    _createInsertPanel: function _createInsertPanel() {
        ...
        {
            title: '选择图片',
            tpl: '<div>\n<div class="w-e-button-container">\n                        <button id="' + linkBtnId + '" class="">选择</button>\n                    </div>\n                </div>',
            events: [{
                selector: '#' + linkBtnId,
                type: 'click',
                fn: function fn() {
        ...
        if (config.showSelectImg) {
            // 显示“网络图片”
            tabsConfigResult.push(tabsConfig[2]);
        }
